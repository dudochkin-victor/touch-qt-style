...
class IdentifierMouseEvent : public QMouseEvent
{
public:
    IdentifierMouseEvent(QMouseEvent::Type pType, const QPoint &position, Qt::MouseButton button, Qt::MouseButtons buttons, Qt::KeyboardModifiers modifiers)
        : QMouseEvent(pType, position, button, buttons, modifiers)
    {}
    IdentifierMouseEvent(QMouseEvent::Type pType, const QPoint &position,  const QPoint & globalPos, Qt::MouseButton button, Qt::MouseButtons buttons, Qt::KeyboardModifiers modifiers)
        : QMouseEvent(pType, position, globalPos, button, buttons, modifiers)
    {}
...
public:
    ...
    bool shouldEventBeIgnored(QEvent *event) const
    {
        // if the dynamic cast doesn't fail, the event was sended by us (i.e. via
        // PressDelayHandler::sendMouseEvent), so stop further recognition handling
        // to avoid recursion
        //
        // to use a boolean variable for the recurse detection handling
        // works fine for just one single event loop
        return (dynamic_cast<IdentifierMouseEvent *>(event));
    }
    ...
    bool released(QEvent *e, bool scrollerIsActive)
    {
        bool result = false; // do not consume this wvent
...
protected:
    ...
    void sendMouseEvent(QMouseEvent *me, int flags = 0)
    {
        if (mouseTarget) {
            ...
            if (me) {
                IdentifierMouseEvent copy(me->type(), mouseTarget->mapFromGlobal(me->globalPos()), me->globalPos(), me->button(), me->buttons(), me->modifiers());
                                                              ...
                                                              QEvent *event)
{
    ...
    } else if (scrollerWasScrolling && scroller->state() == QtScroller::Dragging) {
    ...
    if (!inputType) {
        ...
        case QEvent::GraphicsSceneMouseRelease:
        case QEvent::MouseButtonRelease:
            if (PressDelayHandler::instance()->released(event, scrollerIsActive))
                result |= ConsumeEventHint;
            if (scrollerIsActive)
